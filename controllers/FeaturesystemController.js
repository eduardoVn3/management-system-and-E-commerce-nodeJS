...
var FeatureSystem = db.FeatureSystem
var EnchangeRate = db.EnchangeRate

exports.index = (req, res) => {
	FeatureSystem.findAll(
    
    ).then( featureSystem => {

      return res.send(helper.showAll( featureSystem,200 ))

    });
}

exports.enchangerate = (req, res) => {
	EnchangeRate.findById(1).then( (obj)=>{
		if (obj == null) { return res.send(response.error('No se ha encontrado el registro'),404) }
